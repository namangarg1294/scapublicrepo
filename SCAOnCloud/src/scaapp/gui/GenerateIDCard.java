...
import scaapp.utils.SendAttachmentInEmail;

/**
 *
 * @author Comp10
 */
public class GenerateIDCard extends javax.swing.JFrame {
    ...
    public GenerateIDCard(EmployeeDetails ed) {
        this();
        this.ed = ed;
    ...
    private void initComponents() {
        ...
        jtStudentData.setOpaque(false);
        jtStudentData.setSelectionBackground(new java.awt.Color(102, 204, 255));
        jtStudentData.setSelectionForeground(new java.awt.Color(0, 0, 0));
        jtStudentData.setSelectionMode(javax.swing.ListSelectionModel.SINGLE_SELECTION);
...
//        }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(GenerateIDCard.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                ...
                new GenerateIDCard().setVisible(true);
            }
        });
    }
...
//            }
        ...
        //System.out.println("Output of check statement "+IDCardPrintingDao.checkIsIdCardAlreadyPrinted(studentId.getText().substring(0, 10), jcbatches.getSelectedItem().toString()));
        try {
             System.out.println("Output of check statement "+IDCardPrintingDao.checkIsIdCardAlreadyPrinted(studentId.getText().substring(0, 10), jcbatches.getSelectedItem().toString()));
      
             if(IDCardPrintingDao.checkIsIdCardAlreadyPrinted(studentId.getText().substring(0, 10), jcbatches.getSelectedItem().toString()))
        {
            JOptionPane optionPane = new JOptionPane("This Id card is already printed do you want to print again?", JOptionPane.QUESTION_MESSAGE, JOptionPane.YES_NO_OPTION);
      ...
      //int dialogResult = JOptionPane.showConfirmDialog (null, message+"\n\nWould you like to confirm Payement?","Warning",JOptionPane.YES_NO_OPTION);
        if(dialogResult==null || dialogResult == JOptionPane.NO_OPTION || dialogResult == JOptionPane.CLOSED_OPTION){
          // Saving code here
             ...
             System.out.println("on 704");
            sd = StudentDetailDao.getStudentDetailsByContactNo(studentId.getText().substring(0, 10));
            
            idCPD = IDCardPrintingDao.getPrintingDetails(jcbatches.getSelectedItem().toString());
            idCPD.setActualBatch(jcbatches.getSelectedItem().toString());
            ...
            idCPD.setEmail(sd.getStudentEmail());
            idCPD.setSem(sd.getStudentSem());
            if(idCPD.getBatchId().length()>20)
                idCPD.setBatchId(idCPD.getBatchId().substring(0, 21)+"..");
            if(printingDataList.contains(idCPD)){
                 JOptionPane.showMessageDialog(null, "Already added to list");
                 startSet();
                return;
            }
...
//        }
    ...
    private void print() {
        new IDCardPrinter().printIdCard(printingDataList);
        for(IDCardPrintingDetails idcpd:printingDataList){
            try {
                IDCardPrintingDao.insertInIdPrintingDetails(idcpd.getStudentId(),idcpd.getActualBatch(), ed.getEmp_id());
                System.out.println("Record inserted");
//                new Thread(new Runnable() {
//                    @Override
//                    public void run() {
//                        System.out.println("now sending id card");
//                        String imgName = idcpd.getStudentName()+"_"+idcpd.getEmail()+"_"+idcpd.getCourseName()+".png";
//                        SendAttachmentInEmail.sendMessage("scalivedigital", "9752345221", idcpd.getEmail(), "SCA", "Digital ID card -SCA "+new Date(), "Hi "+idcpd.getStudentName()+","+" PFA your Digital ID card for the "+idcpd.getCourseName()+" course", imgName);
//                    }
//                }).start();
            } 
            catch (SQLException ex) {
